...
public class CsvBatchConfig {
    ...
    public ItemReader<People> reader() throws Exception {
        FlatFileItemReader<People> reader = new FlatFileItemReader<>(); //1 读取文件
        reader.setResource(new ClassPathResource("people.csv")); //2
        reader.setLineMapper(new DefaultLineMapper<People>() {{ //3 数据映射
    ...
    public ItemProcessor<People, People> processor() {
        ...
        processor.setValidator(csvBeanValidator()); //2 自定义数据校验器
        return processor;
    }


    @Bean
    public ItemWriter<People> writer(DataSource dataSource) {//1 写数据
    ...
    @Bean
    public Step step1(StepBuilderFactory stepBuilderFactory, ItemReader<People> reader, ItemWriter<People> writer,
                      ItemProcessor<People, People> processor) {
        return stepBuilderFactory
                .get("step1")
                .<People, People>chunk(65000) //1 每次提交数据量
                .reader(reader) //2
